...
quantum_cnot(int control, int target, quantum_reg *reg)
{
  #ifdef RECORD_ON
  int cpos = 0;
  SET_VALUE_TO_CURR(cnot, cpos);
  if (cpos > -1) {
    char filename[100];
    sprintf(filename, "./bdata/cnot_%d.dat", cpos);
    
    FILE* f = fopen(filename, "wb");
    fwrite(&control, sizeof(int), 1, f);
    fwrite(&target, sizeof(int), 1, f);
    fwrite(reg, sizeof(quantum_reg), 1, f);
    fwrite(reg->node, sizeof(quantum_reg_node), reg->size, f);
    fwrite(reg->hash, sizeof(int), reg->hashw, f);
  }
  #endif
  #ifdef TIMER_ON
  START_TIMER(cnot)
...
quantum_toffoli(int control1, int control2, int target, quantum_reg *reg)
{
  #ifdef RECORD_ON
  int cpos = 0;
  SET_VALUE_TO_CURR(tof, cpos);
  if (cpos > -1) {
    char filename[100];
    sprintf(filename, "./bdata/tof_%d.dat", cpos);

    FILE* f = fopen(filename, "wb");
    fwrite(&control1, sizeof(int), 1, f);
    fwrite(&control2, sizeof(int), 1, f);
    fwrite(&target, sizeof(int), 1, f);
    fwrite(reg, sizeof(quantum_reg), 1, f);
    fwrite(reg->node, sizeof(quantum_reg_node), reg->size, f);
    fwrite(reg->hash, sizeof(int), reg->hashw, f);
...
quantum_sigma_x(int target, quantum_reg *reg)
{
  #ifdef RECORD_ON
  int cpos = 0;
  SET_VALUE_TO_CURR(sig, cpos);
  if (cpos > -1) {
    char filename[100];
    sprintf(filename, "./bdata/sig_%d.dat", cpos);
